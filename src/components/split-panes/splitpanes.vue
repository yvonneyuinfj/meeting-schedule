...
export default {
  ...
  props: {
    /** 窗格拆分的方向 默认情况下为垂直，这意味着拆分器是垂直的，但您可以水平调整大小 */
    horizontal: { type: Boolean },
    /** 拖动分割线到达下一个分隔线时，是否推动下一个分割线。 */
    pushOtherPanes: { type: Boolean, default: false },
    /** 双击分割线以最大化下一个窗格 */
    dblClickSplitter: { type: Boolean, default: false },
    /** 支持从右向左的方向。 */
    rtl: { type: Boolean, default: false },
    /** 设置为true时显示第一个分割线。这允许在分割线双击时最大化第一个窗格。 */
  ...
  provide() {
    return {
      ...
      resizeNextPaneSize: this.resizeNextPaneSize,
      resizeLastPaneSize: this.resizeLastPaneSize,
      checkHasNextPane: this.checkHasNextPane,
  ...
  methods: {
    updatePaneComponents() {
      this.panes.forEach(pane => {
        if (pane.newSize) {
          pane.update &&
            pane.update({
              [this.horizontal ? 'height' : 'width']: `${this.indexedPanes[pane.id].newSize}%`
            });
          this.indexedPanes[pane.id].newSize = null;
        } else {
          pane.update &&
            pane.update({
    ...
    calculatePanesSize(drag) {
      ...
      const paneAfterMaxReached =
        paneAfter.max < 100 &&
        dragPercentage <= 100 - (paneAfter.max + this.sumNextPanesSize(splitterIndex + 1));
    ...
    addSplitter(paneIndex, nextPaneNode, isVeryFirst = false) {
      ...
      const pane = this.panes[splitterIndex];
      const elm = document.createElement('div');
      elm.classList.add('splitpanes__splitter');
      let draggable = true;
      if (pane) {
        draggable = pane.draggable;
      }
      if (draggable) {
        if (!isVeryFirst) {
    ...
    /** 折叠收起时同时需要修改下一个pane的尺寸 */
    resizeNextPaneSize({ target, changeSize }) {
      const pane = this.indexedPanes[target._.uid];
      if (pane.index < this.panes.length) {
        this.panes[pane.index + 1].size = this.panes[pane.index + 1].size + parseFloat(changeSize);
      }
    },
    /** 折叠收起时同时需要修改上一个pane的尺寸 */
    resizeLastPaneSize({ target, changeSize }) {
      const pane = this.indexedPanes[target._.uid];
      if (pane.index > 0) {
        this.panes[pane.index - 1].size = this.panes[pane.index - 1].size + parseFloat(changeSize);
      }
    ...
    /** 判断是否存在下一个pane */
    checkHasNextPane({ target }) {
      const pane = this.indexedPanes[target._.uid];
      if (pane.index == this.panes.length - 1) {
        return false;
      } else {
        return true;
      }
    ...
    getSplitSize() {
      this.splitWidth = this.$refs.container.clientWidth;
      this.splitHeight = this.$refs.container.clientHeight;
      return { splitHeight: this.splitHeight, splitWidth: this.splitWidth };
    },
    /** 重新根据具体px值计算百分比 */
    setSize(pane) {
      if (pane.pxSize !== null) {
        if (pane.pxFlag > 0) {
          ...
          if (this.horizontal) {
            //水平布局高度计算百分比
            realSize = (parseFloat(pane.pxSize) / parseFloat(splitHeight)) * 100;
          } else {
            //垂直布局宽度计算百分比
            realSize = (parseFloat(pane.pxSize) / parseFloat(splitWidth)) * 100;
          }
          realSize = realSize.toFixed(2);
          pane.newSize = realSize;
          pane.size = realSize;
        }
      }
    },
    ...
    onPaneAdd(pane) {
      ...
      this.panes.splice(index, 0, {
        ...
        pxSize: pane.size,
        pxFlag: pane.size === null ? null : pane.size.indexOf('px'),
        givenSize: pane.size,
        draggable: pane.draggable,
        update: pane.update
      });
    ...
    initialPanesSizing() {
      ...
      if (leftToAllocate > 0.1) {
        this.panes.forEach(pane => {
          if (pane.size === null) {
            pane.size = Math.max(
              Math.min(leftToAllocate / (this.panesCount - definedSizes), pane.max),
              pane.min
            );
          }
    ...
    equalizeAfterAddOrRemove({ addedPane } = {}) {
      ...
      this.panes.forEach(pane => {
        if (!addedPane || !addedPane.givenSize || addedPane.id !== pane.id) {
          pane.size = Math.max(Math.min(equalSpace, pane.max), pane.min);
        }
  ...
  mounted() {
    ...
    this.ready = true;
    window.onresize = () => {
      //针对 pane中存在设置了具体宽高的需要监听窗口变化重新计算宽高对应的百分比数值
      const filterData = this.panes.filter(item => item.pxFlag > 0);
      if (filterData && filterData.length > 0) {
        this.panes.forEach(pane => {
          // 循环清空初始化px转换为百分比设置的尺寸
          pane.newSize = null;
          if (!pane.pxSize) {
            //清空页面配置中未配置尺寸的pane（这些pane会在页面初始化的时候组件为其计算百分比）
            pane.size = null;
          }
        });
        this.resetPaneSizes();
...
.splitpanes__pane {
  ...
  .goLeft {
    ...
    border-right: 12px solid @layout-body-background;
  ...
  .goTop {
    ...
    border-bottom: 12px solid @layout-body-background;
  ...
  .goRight {
    ...
    border-top: 8px solid transparent;
    border-bottom: 8px solid transparent;
    border-left: 12px solid @layout-body-background;
    cursor: pointer;
    .avic-icon {
      position: absolute;
      top: -6px;
  ...
  .goBottom {
    ...
    margin-left: -24px;
    border: 0;
    border-top: 12px solid @layout-body-background;
    border-right: 8px solid transparent;
    border-left: 8px solid transparent;
    cursor: pointer;
    .avic-icon {
...
.splitpanes {
  .splitpanes__splitter {
    ...
    &::after {
      ...
      top: 50%;
      left: 50%;
      background-color: @text-color-less;
