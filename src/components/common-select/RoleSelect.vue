...
<template>
  <a-modal
    ...
    wrap-class-name="common-content role-select-content vxe-table--ignore-clear"
    :centered="true"
    :visible="visible"
          ...
          >
            <template #bodyCell="{ record }">
              <span class="user-icon">
                <avic-icon color="#24b3b3" svg="avic-account-box-fill" />
              </span>
              <span class="cell-txt" :title="record['roleName']">
          ...
          >
            <template #bodyCell="{ column, text, record }">
              <template v-if="column.key === 'roleName'">
                <span class="" :title="record['roleName']">
                  <avic-icon svg="avic-account-box-fill" color="#24b3b3" />
                  {{ text }}
...
const props = defineProps({
  ...
  /** 右侧列表已选标题文案 */
  name: { type: String, default: '角色' },
  /** 标题 */
  title: { type: String, default: '角色选择' },
  /** 辅助提示信息 */
  titleTips: { type: String, default: '' },
  /** 搜索提示 */
  searchPlaceholder: { type: String, default: '请输入角色名称或角色编码' },
  /** 单选 single ，多选 multi */
  ...
  newV => {
    if (newV && newV.ids) {
      ...
      if (orgIdentityNames.length == 0) {
        httpRequest(getInitRoleInfoUrl.value, ids, getInitRoleInfoMethod.value)
          .then(res => {
            if (res.data) {
              res.data.map(item => {
...
function getList() {
  let param = {
    ...
    parameter: {
      ...
      sord: sord.value,
      // 扩展自定义参数 2022-3-14 By Cj
      param: {
        ...customParam.value
      }
    },
    method: roleMethodType.value
  };
...
function handleTableChange(paginationInfo, filters, sorter) {
  pagination.current = paginationInfo.current; // 页数
  pagination.pageSize = paginationInfo.pageSize; // 每页条数
  // 处理排序信息
  if (sorter) {
...
/**
  加载默认值 判断是否需要去查询数据库
  */
// function loadingDefaultData(value, num) {
//   if (value) {
//     let isLoading = false;
//     if (value && value.ids) {
//       let ids = (value + '').split(';');
...
function setSelect(res, isCallBack = false) {
  if (defaultSelectedRows.value.length > 0) {
...
/**
   加载角色列表
   */
function finInitRoleInfo(success) {
  columns.value = roleColumns;
  getList().then(res => {
    if (res && success) {
      success(res);
    }
...
function confirm() {
  const result = getResult();
  if (result.ids) {
    $emit('callBack', result, select.value); //已选中项的完整数据
  } else {
    $emit('callBack', {});
  }
  visible.value = false;
