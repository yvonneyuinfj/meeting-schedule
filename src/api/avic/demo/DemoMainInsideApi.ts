...
const basePath = '/demo/template/demomaininsides';
/** 主子表内嵌 */
export interface DemoMainInsideDto extends BaseBeanModel {
  /** 必填 */
  demoNotnull?: string;
  /** 用户 */
  demoUserId?: string;
...
/** 获取分页数据 */
export function listDemoMainInsideByPage(
  param: QueryParamModel
): Promise<ResponsePageData<DemoMainInsideDto>> {
  return request.post(basePath + '/search-by-page/v1', param);
}

/** 根据id加载数据 */
export function getDemoMainInside(id: [string]): Promise<ResponseBaseData<DemoMainInsideDto>> {
  return request.get(basePath + '/get/' + id + '/v1');
}

/** 保存表单数据 */
export function saveDemoMainInside(form: DemoMainInsideDto): Promise<ResponseBaseData<any>> {
  return request.post(basePath + '/save/v1', form);
}

/** 根据id集合删除数据 */
export function delDemoMainInside(ids: [string]): Promise<ResponseBaseData<any>> {
