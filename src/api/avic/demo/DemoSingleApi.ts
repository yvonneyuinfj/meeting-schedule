...
const basePath = '/demo/template/demosingles';
/** 单表 */
export interface DemoSingleDto extends BaseBeanModel {
  /** 必填 */
  demoNotnull?: string;
  /** 用户 */
  demoUserId?: string;
...
): Promise<ResponsePageData<DemoSingleDto>> {
  return request.post(basePath + '/search-by-page/v1', param);
}

/** 根据id加载数据 */
export function getDemoSingle(id: string): Promise<ResponseBaseData<DemoSingleDto>> {
  return request.get(basePath + '/get/' + id + '/v1');
}

/** 保存表单数据 */
export function saveDemoSingle(form: DemoSingleDto): Promise<ResponseBaseData<any>> {
  return request.post(basePath + '/save/v1', form);
}

/** 根据id集合删除数据 */
export function delDemoSingle(ids: [string]): Promise<ResponseBaseData<any>> {
