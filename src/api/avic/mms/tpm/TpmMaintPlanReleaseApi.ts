...
import { TpmMaintPlanDto } from '@/api/avic/mms/tpm/TpmMaintPlanApi';

const basePath = '/mms/tpm/tpmmaintplanrelease';

/** 保养计划表 */
export interface TpmMaintPlanReleaseDto extends BaseBeanModel {
    /** 设备标准主表ID */
...
): Promise<ResponsePageData<TpmMaintPlanReleaseDto>> {
    return request.post(basePath + '/search-by-page/v1', param);
}

/** 保存表单数据 */
export function saveTpmMaintPlan(form: [TpmMaintPlanReleaseDto]): Promise<ResponseBaseData<any>> {
...
export function approvalMaintPlanFeedBack(data) {
    return request.post(basePath + '/start-feed-back-process/v1', data);
}

export function listTpmMaintPlanFeedBackByPage(
    param: QueryParamModel
): Promise<ResponsePageData<TpmMaintPlanDto>> {
    return request.post(basePath + '/search-feed-back-by-page/v1', param);
}
