...
const basePath = '/mms/mds/mdsprojectsimplifys';
/** 项目定义表 */
export interface MdsProjectSimplifyDto extends BaseBeanModel {
  /** 项目编号 */
  projectCode?: string;
  /** 项目名称 */
  projectName?: string;
  ...
  currencyType?: any;
  /** 上级项目ID */
  parentMdsProjectSimplifyId?: string;
  /** 是否实物生产 */
  ynProduction?: any;
  /** 项目所属行业 */
  industryType?: any;
...
/** 获取分页数据 */
export function listMdsProjectSimplifyByPage(
  param: QueryParamModel
): Promise<ResponsePageData<MdsProjectSimplifyDto>> {
  return request.post(basePath + '/search-by-page/v1', param);
}

/** 根据id加载数据 */
export function getMdsProjectSimplify(id: string): Promise<ResponseBaseData<MdsProjectSimplifyDto>> {
  return request.get(basePath + '/get/' + id + '/v1');
}

/** 保存表单数据 */
export function saveMdsProjectSimplify(form: MdsProjectSimplifyDto): Promise<ResponseBaseData<any>> {
  return request.post(basePath + '/save/v1', form);
}

/** 根据id集合删除数据 */
export function delMdsProjectSimplify(ids: [string]): Promise<ResponseBaseData<any>> {
