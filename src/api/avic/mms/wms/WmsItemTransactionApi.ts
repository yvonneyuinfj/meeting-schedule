...
const basePath = '/mms/wms/wmsitemtransactions';
/** 出入库明细表 */
export interface WmsItemTransactionDto extends BaseBeanModel {
  /** 事务处理流水号  */
...
/** 获取分页数据 */
export function listWmsItemTransactionByPage (
  param: QueryParamModel
): Promise<ResponsePageData<WmsItemTransactionDto>> {
  return request.post(basePath + '/search-by-page/v1', param);
}

/** 根据id加载数据 */
export function getWmsItemTransaction (id: string): Promise<ResponseBaseData<WmsItemTransactionDto>> {
  return request.get(basePath + '/get/' + id + '/v1');
}

/** 保存表单数据 */
export function saveWmsItemTransaction (form: WmsItemTransactionDto): Promise<ResponseBaseData<any>> {
  return request.post(basePath + '/save/v1', form);
}

/** 根据id集合删除数据 */
export function delWmsItemTransaction (ids: [string]): Promise<ResponseBaseData<any>> {
