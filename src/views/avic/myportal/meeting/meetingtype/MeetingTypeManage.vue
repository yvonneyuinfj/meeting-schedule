                ...
                <a-dropdown :trigger="['contextmenu']">
                  ...
                  <template #overlay>
                    <a-menu @click="({ key: menuKey }) => handleContextMenuClick(dataRef, menuKey)">
                      <a-menu-item key="1" v-if="getContextMenuVisiable(dataRef, '1') && proxy.$hasPermi(['meetingType:add'])">
                        添加平级节点
                      </a-menu-item>
                      <a-menu-item key="2" v-if="proxy.$hasPermi(['meetingType:add'])">
                        添加子节点
                      </a-menu-item>
                      <a-menu-item key="3" v-if="proxy.$hasPermi(['meetingType:edit'])">
                        编辑节点
                      </a-menu-item>
                      <a-menu-item key="4" v-if="proxy.$hasPermi(['meetingType:del'])">
                        删除节点
                      </a-menu-item>
                    </a-menu>
                  </template>
...
import {
  ...
  searchTreeList,
  delMeetingType
} from '@/api/avic/myportal/meeting/MeetingTypeApi'; // 引入模块API
import {
  addSameNode,
  addSubNode,
  getExpandedKeys,
...
import MeetingTypeAdd from './MeetingTypeAdd.vue'; // 引入添加页面组件
import MeetingTypeEdit from './MeetingTypeEdit.vue'; // 引入编辑页面组件
import MeetingTypeDetail from './MeetingTypeDetail.vue'; // 引入详细页面组件
...
const meetingTypeDetailRef = ref(null);
onMounted(() => {
  // 加载树数据
  getTreeList();
  // 重置树高度
...
function afterEditTreeNode(data) {
  if (currentSelectedNode.value.dataRef.parentId != data.parentId) {
    ...
    // 节点编辑成功后重新加载右侧详细区数据
    meetingTypeDetailRef.value.reloadDetail();
  }
}
/** 树节点删除方法 */
  ...
  proxy.$confirm({
    ...
    onOk: () => {
      if (selectNodeData.id) {
        delMeetingType(selectNodeData.id)
          .then(res => {
            if (res.success) {
