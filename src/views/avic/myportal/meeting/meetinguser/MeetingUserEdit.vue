...
import type { MeetingUserDto } from '@/api/avic/myportal/meeting/MeetingUserApi'; // 引入模块DTO
import { listMeetingUserByPage } from '@/api/avic/myportal/meeting/MeetingUserApi'; // 引入模块API
...
const queryForm = ref<MeetingUserDto>({});
...
const validateRules = {
  userId: [
    { required:true, message: 'USER_ID列不能为空' }
  ],
  secretLevel: [
    { required:true, message: 'SECRET_LEVEL列不能为空' }
  ]
}; // 必填列,便于保存和新增数据时校验
...
function getList() {
  ...
  selectedRows.value = [];
  loading.value = true;
  queryForm.value.meetingId = props.mainId ? props.mainId : '-1';
  queryParam.searchParams = queryForm.value;
  listMeetingUserByPage(queryParam)
    .then(response => {
...
function validateRecordData(records) {
  ...
  for (let index in records) {
    flag = proxy.$validateRecordData(records[index], validateRules, list.value, meetingUser);
    if (!flag) {
      break;
    }
  }
