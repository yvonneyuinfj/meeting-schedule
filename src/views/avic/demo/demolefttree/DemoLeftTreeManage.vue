<template>
  <AvicSplit @onresize="onResize">
                  ...
                  <template #overlay>
                    <a-menu @click="({ key: menuKey }) => handleContextMenuClick(dataRef, menuKey)">
                      <a-menu-item
                        key="1"
                        v-if="
                          getContextMenuVisiable(dataRef, '1') &&
                          proxy.$hasPermi(['demoLeftTree:add'])
                        "
                      >
                        添加平级节点
                      </a-menu-item>
    ...
    <AvicPane>
      <DemoRightListManage ref="rightListManageRef" :mainId="nodeId" />
    </AvicPane>
    <!-- 添加页面弹窗 -->
    <DemoLeftTreeAdd
...
import { getTreeData, searchTreeList, delDemoLeftTree } from '@/api/avic/demo/DemoLeftTreeApi'; // 引入模块API
import {
  addSameNode,
  addSubNode,
  getExpandedKeys,
...
import DemoLeftTreeAdd from './DemoLeftTreeAdd.vue'; // 引入添加页面组件
import DemoLeftTreeEdit from './DemoLeftTreeEdit.vue'; // 引入编辑页面组件
import DemoRightListManage from '../demorightlist/DemoRightListManage.vue'; // 引入子表页面组件
  ...
  proxy.$confirm({
    ...
    onOk: () => {
      if (selectNodeData.id) {
        delDemoLeftTree(selectNodeData.id)
          .then(res => {
            if (res.success) {
