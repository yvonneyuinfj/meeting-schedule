...
} from '@/api/avic/demo/DemoMainInsideFlowApi'; // 引入模块API
import {
...
export function useDemoMainInsideFlowForm({ props: props, emit: emit }) {
  const { proxy } = getCurrentInstance();
  const form = ref<DemoMainInsideFlowDto>({});
  const formRef = ref(null);
  const formCode = 'DemoMainInsideFlowVue3';
  ...
  const demoSubInsideFlowEdit = ref();
  const layout = {
    labelCol: { flex: '140px' },
    wrapperCol: { flex: '1' }
  };
      ...
      .then(async () => {
        demoSubInsideFlowEdit.value
          .validate(async validate => {
            ...
            loading.value = true;
            const subInfoList = demoSubInsideFlowEdit.value.getChangedData(); // 获取子表数据
            // 处理数据
  ...
  async function saveAndStartProcess(params) {
    ...
    if (!props.bpmInstanceObject && !params.dbid) {
      ...
      loading.value = true;
      // 处理数据
      const bean = proxy.$lodash.cloneDeep(form.value);
      bean.demoCheckbox = bean.demoCheckbox?.join(';'); // 处理复选框数据
      bean.demoSelectMultiple = bean.demoSelectMultiple?.join(';'); // 处理多选下拉数据
      bean.demoSwitch = bean.demoSwitch === 'Y' ? 'Y' : 'N'; // 处理开关
      bean.demoSubFlowList = subInfoList; //挂载子表数据
      if (autoCode.value) {
        // 处理编码
        const codeResult = await autoCode.value.submitCode();
        bean.demoCode = codeResult.autoCodeValue;
      }
      const param = {
