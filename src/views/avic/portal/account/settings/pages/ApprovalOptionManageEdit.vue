...
import {
  insertOrUpdateApprovalOptionList,
  EditApprovalOptionList
...
const rules: Record<string, Rule[]> = {
    ...
    processName: [{ required: true, message: '请选择流程！', trigger: 'blur' }],
...
onMounted(() => {
  getApprovalManageUser(props.formId);
});
 ...
 */
function getApprovalManageUser(id) {
  loading.value = true;
  EditApprovalOptionList(id)
    .then((res: any) => {
      if (res.success) {
        form.value = res.data;
        loading.value = false;
