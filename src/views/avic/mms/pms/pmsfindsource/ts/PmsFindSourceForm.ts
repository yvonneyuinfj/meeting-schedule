...
export function usePmsFindSourceForm({ props: props, emit: emit }) {
  ...
  const form = ref<PmsFindSourceDto>({pmsMethod: '0', pmsPrice: '0'});
  ...
  const rules: Record<string, Rule[]> = {
    ...
    projectTopicNumber: [
      { required: true, message: '确定供应商理由不能为空', trigger: 'change' }
    ],
    productReqClassify: [
  ...
  const pmsMethodList = ref([]); // 采购方式通用代码
  const pmsPriceList = ref([]); // 采购价格通用代码
  const secretLevelList = ref([]); // 密级通用代码
  const lookupParams = [
    { fieldName: 'pmsMethod', lookUpType: 'PMS_METHOD' },
  ...
  function getLookupList () {
    ...
    pmsMethodList.value = result.pmsMethod;
