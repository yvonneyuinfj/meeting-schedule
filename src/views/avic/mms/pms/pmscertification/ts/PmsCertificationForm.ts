...
import { getPmsCertification, savePmsCertification, saveFormAndStartProcess } from '@/api/avic/mms/pms/PmsCertificationApi'; // 引入模块API
import {
  startFlowByFormCode,
  closeFlowLoading,
  openFlowDetail,
...
export function usePmsCertificationForm({ props: props, emit: emit }) {
  const { proxy } = getCurrentInstance();
  const form = ref<PmsCertificationDto>({});
  ...
  form.value.pmsCheckBillId = props.mainId;
  onMounted(() => {
    // 获取通用代码
    getLookupList();
    // 获取当前用户对应的文档密级
