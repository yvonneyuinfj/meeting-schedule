...
import {
  getPmsProcurementInformationReleaseApplication,
  saveFormAndStartProcess,
  savePmsProcurementInformationReleaseApplication
...
export function usePmsProcurementInformationReleaseApplicationForm({ props: props, emit: emit }) {
  const { proxy } = getCurrentInstance();
  const form = ref<PmsProcurementInformationReleaseApplicationDto>({});
  const formRef = ref(null);
  const formCode = 'PmsProcurementInformationReleaseApplication';
  ...
  onMounted(() => {
    ...
    if (props.formId || form.value.id) {
      ...
      // 添加表单的初始值
      initForm();
      form.value.publishingDept = '中国航发沈阳发动机研究所';
      form.value.affiliatedLargeDept = '中国航空发动机集团有限公司';
      form.value.email = '11111@ac.com';
    }
  });
  ...
  function getFormData(id) {
    ...
    loading.value = true;
    getPmsProcurementInformationReleaseApplication(id)
      .then(async res => {
        if (res.success) {
          if (res.data) {
            form.value = res.data;
            // 处理数据
            form.value.qualificationRequirements = form.value.qualificationRequirements?.toString()?.split(';'); // 处理复选框数据
            form.value.professionalField = form.value.professionalField?.toString()?.split(';'); // 处理复选框数据
            loading.value = false;
          } else {
            initForm();
            loading.value = false;
  ...
  function saveForm(params) {
    formRef.value
      ...
      .then(() => {
        ...
        postData.qualificationRequirements = postData.qualificationRequirements?.join(';'); // 处理复选框数据
        postData.professionalField = postData.professionalField?.join(';'); // 处理复选框数据
        // 发送请求
        savePmsProcurementInformationReleaseApplication(postData)
          .then(res => {
            if (res.success) {
