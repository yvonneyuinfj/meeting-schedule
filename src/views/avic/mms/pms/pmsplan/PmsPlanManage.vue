    ...
    <div class="table-wrapper">
      ...
      >
        ...
        <template #bodyCell="{ column, text, record, index }">
          ...
          <template v-else-if="column.dataIndex === 'mergeFlagName' && record.mergeFlag === '5'">
...
// import PmsPlanDetail from './PmsPlanDetail.vue'; // 引入详情页面组件
import PmsPlanBkManage from './PmsPlanBkManage.vue'; // 引入详情页面组件
import PmsRequireReceiveSelect from './PmsRequireReceiveSelect.vue'; // 引入科研需求选择页面组件
...
const showAddModal = ref(false); // 是否展示合并计划弹窗
const showEditModal = ref(false); // 是否展示编辑弹窗
const showDetailModal = ref(false); // 是否展示详情弹窗
const open = ref(false); // 是否展示科研需求选择弹窗
// const showImportModal = ref(false); // 是否展示导入弹窗
// const excelParams = ref({ tableName: 'pmsPlan' }); // 导入Excel数据过滤参数
...
const pmsRequireReceiveSelect = ref(null);
const lookupParams = [
...
function handleEdit() {
  ...
  // if (selectedRows.value[0].mergeFlag !== '5') {
  //   proxy.$message.warning('只有合并后的数据才可以编辑！');
  //   return;
  // }
...
const handleOk = () => {
  const info = pmsRequireReceiveSelect.value.selectedRowKeys;
  if (info.length > 0) {
    savePmsPlanList(info)
        .then(res => {
          if (res.success) {
    ...
    proxy.$message.warning('请选择科研需求数据');
    open.value = false;
  }
};
