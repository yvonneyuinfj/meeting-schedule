<template>
  <div class="content-wrapper">
        ...
        <a-row :gutter="16">
          ...
          <a-col v-bind="colLayout.cols">
            <a-form-item label="检验项目类型">
              <a-select
                v-model:value="queryForm.checkItemType"
                :get-popup-container="triggerNode => triggerNode.parentNode"
                option-filter-prop="children"
                :show-search="true"
                :allow-clear="true"
          ...
          <a-col v-bind="colLayout.cols" v-show="advanced">
            <a-form-item label="检验项目代码">
              <a-input
                v-model:value="queryForm.checkItemCode"
          ...
          <a-col v-bind="colLayout.cols" v-show="advanced">
            <a-form-item label="状态">
              <a-select
                v-model:value="queryForm.ynEnable"
                :get-popup-container="triggerNode => triggerNode.parentNode"
                option-filter-prop="children"
                :show-search="true"
                :allow-clear="true"
      ...
      >
        <template #toolBarLeft>
          <a-space>
            ...
            <!--            <a-button-->
            <!--              v-hasPermi="['pmsCheckItem:import']"-->
            <!--              title="导入"-->
            <!--              type="primary"-->
            <!--              ghost-->
            <!--              @click="handleImport">-->
            ...
            <!--            </a-button>-->
            <!--            <a-button-->
            <!--              v-hasPermi="['pmsCheckItem:export']"-->
            <!--              title="导出"-->
            <!--              type="primary"-->
            <!--              ghost-->
            <!--              @click="handleExport">-->
    ...
    <AvicExcelImport
      ...
      title="单表模板导入"
      importUrl="/mms/pms/pmscheckitems/importData/v1"
      downloadTemplateUrl="/mms/pms/pmscheckitems/downloadTemplate/v1"
      @reloadData="getList"
...
import type {PmsCheckItemDto} from '@/api/avic/mms/pms/PmsCheckItemApi'; // 引入模块DTO
import {
  listPmsCheckItemByPage,
  delPmsCheckItem,
  exportExcel,
  startPmsCheckItem,
  stopPmsCheckItem
} from '@/api/avic/mms/pms/PmsCheckItemApi'; // 引入模块API
import PmsCheckItemAdd from './PmsCheckItemAdd.vue'; // 引入添加页面组件
import PmsCheckItemEdit from './PmsCheckItemEdit.vue'; // 引入编辑页面组件
import PmsCheckItemDetail from './PmsCheckItemDetail.vue'; // 引入详情页面组件
...
const queryForm = ref<PmsCheckItemDto>({});
...
const excelParams = ref({tableName: 'pmsCheckItem'}); // 导入Excel数据过滤参数
...
const ynEnableList = ref([]); // 状态通用代码
const lookupParams = [
  {fieldName: 'checkItemType', lookUpType: 'PMS_CHECK_ITEM_TYPE '},
  {fieldName: 'ynEnable', lookUpType: 'POM_TMPL_STATUS'}
];
...
function getList() {
  ...
  loading.value = true;
  listPmsCheckItemByPage(queryParam)
    .then(response => {
...
function getLookupList() {
  proxy.$getLookupByType(lookupParams, result => {
    ...
    ynEnableList.value = result.ynEnable;
  });
}
