<template>
  <AvicModal :visible="true" title="添加" width="960px" height="520px" :centered="true" @cancel="closeModal"
    ...
    <a-spin :spinning="loading">
        ...
        <a-row :gutter="0">
          <a-col v-bind="colLayout.cols">
            <a-form-item name="billNo" label="单据号">
              <!--              <avic-auto-code v-model:value="form.billNo" ref="autoCode" code-type="TPM_YEAR_MAINT_MODIFY_PLAN_NO"-->
              <!--                code-param="TPM_YEAR_MAINT_MODIFY_PLAN" :allow-clear="true" :disabled="false" placeholder="请输入单据号" />-->
              <a-input v-model:value="form.billNo" default-value="自动生成" disabled/>
            </a-form-item>
          </a-col>
          <a-col v-bind="colLayout.cols">
          ...
          <a-col v-bind="colLayout.cols">
            <a-form-item name="editDate" label="填报日期">
              <a-input v-model:value="form.editDate" format="YYYY-MM-DD" value-format="YYYY-MM-DD"
                       placeholder="默认当前时间"
                       disabled :defaultValue="currentTime"/>
            </a-form-item>
          </a-col>
          <a-col v-bind="colLayout.cols">
      ...
      <TpmYearMaintModifyPlanLEdit ref="tpmYearMaintModifyPlanLEdit"/>
    </a-spin>
    <template #footer>
      <a-button title="保存" type="primary" :loading="loading" @click="saveForm">保存</a-button>
      <a-button title="保存并启动流程" type="primary" :loading="processLoading" @click="saveAndStartProcess">保存并启动流程
      </a-button>
