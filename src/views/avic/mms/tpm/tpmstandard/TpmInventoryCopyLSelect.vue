...
import { standardListTpmInventoryCopyLSelectByPage } from '@/api/avic/mms/tpm/TpmInventoryApi';
...
const selectedRowIdKeys = ref([]); // 选中数据主键集合
...
function getList() {
  ...
  selectedRowIdKeys.value = []; // 清空选中
  selectedRows.value = []; // 清空选中
  loading.value = true;
  queryParam.searchParams.tpmStandardId = props.formId;
  queryParam.searchParams.useDeptId = proxy.$getLoginUser().entityDeptId;
  standardListTpmInventoryCopyLSelectByPage(queryParam)
      .then(response => {
...
function onSelectChange(rowKeys, rows) {
  ...
  rows.map(item => {
    selectedRowIdKeys.value.push(item.tpmStandardId);
  });
