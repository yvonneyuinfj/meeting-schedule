...
import { getTpmCaseShare, saveTpmCaseShare, saveFormAndStartProcess } from '@/api/avic/mms/tpm/TpmCaseShareApi'; // 引入模块API
...
export function useTpmCaseShareForm({ props: props, emit: emit }) {
  const { proxy } = getCurrentInstance();
  const form = ref<TpmCaseShareDto>({});
  const formRef = ref(null);
  const formCode = 'TpmCaseShare';
  ...
  function initForm() {
    ...
    //form.value.note = "初始化";
    //初始化分享时间
    //let shareTime = formatDate(new Date(), 'YYYY-MM-DD HH:mm:ss');
    //form.value.shareDate = shareTime;

    //let userInfoss = sessionStorage.getItem("userId");
    //alert(userInfoss);
  }
  /** 校验通过后，读取要启动的流程模板 */
