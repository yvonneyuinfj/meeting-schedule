...
import { getTpmYearMaintModifyPlanAll, saveTpmYearMaintModifyPlanAll, saveFormAndStartProcess } from '@/api/avic/mms/tpm/TpmYearMaintModifyPlanAllApi'; // 引入模块API
import {
...
export function useTpmYearMaintModifyPlanAllForm({ props: props, emit: emit }) {
  const { proxy } = getCurrentInstance();
  const form = ref<TpmYearMaintModifyPlanAllDto>({});
  const formRef = ref(null);
  const formCode = 'TpmYearMaintModifyPlanAll';
  ...
  function getFormData(id) {
    ...
    loading.value = true;
    getTpmYearMaintModifyPlanAll(id)
      .then(async res => {
        if (res.success) {
          if (res.data) {
            form.value = res.data;
  ...
  function saveForm(params) {
    formRef.value
      ...
      .then(() => {
        ...
        // 发送请求
        saveTpmYearMaintModifyPlanAll(postData)
          .then(res => {
            if (res.success) {
