                ...
                <a-dropdown :trigger="['contextmenu']">
                  ...
                  <template #overlay>
                    <a-menu @click="({ key: menuKey }) => handleContextMenuClick(dataRef, menuKey)">
                      <a-menu-item key="1" v-if="getContextMenuVisiable(dataRef, '1') && proxy.$hasPermi(['tpmArea:add'])">
                        添加平级节点
                      </a-menu-item>
                      <a-menu-item key="2" v-if="proxy.$hasPermi(['tpmArea:add'])">
                        添加子节点
                      </a-menu-item>
                      <a-menu-item key="3" v-if="proxy.$hasPermi(['tpmArea:edit'])">
                        编辑节点
                      </a-menu-item>
                      <a-menu-item key="4" v-if="proxy.$hasPermi(['tpmArea:del'])">
                        删除节点
                      </a-menu-item>
...
import {
  ...
  searchTreeList,
  delTpmArea
} from '@/api/avic/mms/tpm/TpmAreaApi'; // 引入模块API
...
import TpmAreaAdd from './TpmAreaAdd.vue'; // 引入添加页面组件
import TpmAreaEdit from './TpmAreaEdit.vue'; // 引入编辑页面组件
import TpmAreaDetail from './TpmAreaDetail.vue'; // 引入详细页面组件
import TpmAreaList from './TpmAreaList.vue'; // 引入列表页面组件 2023-11-10
...
const tpmAreaDetailRef = ref(null);
...
function afterEditTreeNode(data) {
  if (currentSelectedNode.value.dataRef.parentId != data.parentId) {
    ...
    // 节点编辑成功后重新加载右侧详细区数据
    tpmAreaDetailRef.value.reloadDetail();
  }
}
/** 树节点删除方法 */
