  ...
  <AvicSplit @onresize="onResize">
    <AvicPane size="250px" collapsible="left">
      <div class="tree_content" ref="treeContentRef">
        <div class="surface-title">设备大类维护</div>
        <div class="surface-search">
          <a-input-search
            :allow-clear="true"
            placeholder="输入大类名称查询"
                ...
                <a-dropdown :trigger="['contextmenu']">
                  ...
                  <template #overlay>
                    <a-menu @click="({ key: menuKey }) => handleContextMenuClick(dataRef, menuKey)">
                      <a-menu-item key="1" v-if="getContextMenuVisiable(dataRef, '1') && proxy.$hasPermi(['tpmAssetClass:add'])">
                        添加平级节点
                      </a-menu-item>
                      <a-menu-item key="2" v-if="proxy.$hasPermi(['tpmAssetClass:add'])">
                        添加子节点
                      </a-menu-item>
                      <a-menu-item key="3" v-if="proxy.$hasPermi(['tpmAssetClass:edit'])">
                        编辑节点
                      </a-menu-item>
                      <a-menu-item key="4" v-if="proxy.$hasPermi(['tpmAssetClass:del'])">
                        删除节点
                      </a-menu-item>
                    </a-menu>
                  </template>
...
import {
  ...
  searchTreeList,
  delTpmAssetClass
} from '@/api/avic/mms/tpm/TpmAssetClassApi'; // 引入模块API
import {
  addSameNode,
  addSubNode,
  getExpandedKeys,
...
import TpmAssetClassAdd from './TpmAssetClassAdd.vue'; // 引入添加页面组件
import TpmAssetClassEdit from './TpmAssetClassEdit.vue'; // 引入编辑页面组件
import TpmAssetClassDetail from './TpmAssetClassDetail.vue'; // 引入详细页面组件
...
const tpmAssetClassDetailRef = ref(null);
onMounted(() => {
  // 加载树数据
  getTreeList();
  // 重置树高度
...
function afterEditTreeNode(data) {
  if (currentSelectedNode.value.dataRef.parentId != data.parentId) {
    ...
    // 节点编辑成功后重新加载右侧详细区数据
    tpmAssetClassDetailRef.value.reloadDetail();
  }
}
/** 树节点删除方法 */
  ...
  proxy.$confirm({
    ...
    onOk: () => {
      if (selectNodeData.id) {
        delTpmAssetClass(selectNodeData.id)
          .then(res => {
            if (res.success) {
