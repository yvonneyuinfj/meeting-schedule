<template>
  <div class="content-wrapper">
          ...
          <a-col v-bind="colLayout.cols" v-show="advanced">
            <a-form-item label="培训人员">
              <AvicCommonSelect
                  ...
                  type="userSelect"
                  placeholder="请选择培训人员"
                  :defaultShowValue="queryForm.trainingPersonIdAlias"
                  @callback="
                  result => {
                    queryForm.trainingPersonIdAlias = result.names;
                  }
                "
              />
            </a-form-item>
    ...
    <div class="table-wrapper">
      <AvicTable
          ref="tpmOplTrainingQuery"
          table-key="tpmOplTrainingQuery"
          :columns="columns"
      ...
      >
        <template #toolBarLeft>
          <a-space>
            <a-button
                v-hasPermi="['tpmOplTrainingQuery:export']"
                title="导出"
                type="primary"
                ghost
                @click="handleExport">
...
import { listTpmOplTrainingByPage, exportExcel } from '@/api/avic/mms/tpm/TpmOplTrainingApi'; // 引入模块API
...
const columns = [
  ...
  {
    title: '培训人员',
    dataIndex: 'trainingPersonIdAlias',
    ellipsis: true,
    minWidth: 120,
    resizable: true,
    align: 'left'
...
function getList() {
  ...
  loading.value = true;
  if (!isUndefined(queryParam.searchParams) && !isUndefined(queryParam.searchParams.subjectCategory)) {
    queryParam.searchParams.subjectCategory = queryParam.searchParams.subjectCategory.toString();
  }
