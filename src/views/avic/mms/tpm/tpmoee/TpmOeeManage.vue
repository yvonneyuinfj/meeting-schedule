<template>
    <AvicSplit horizontal>
        <AvicPane size="55">
                        ...
                        <a-row :gutter="16">
                            <a-col v-bind="colLayout.cols">
                                <a-form-item label="申报月份(起)">
                                    <a-date-picker v-model:value="queryForm.reportDateBegin" format="YYYY-MM"
                                        value-format="YYYY-MM" placeholder="请选择申报月份(起)" picker="month"
                                        :disabled-date="startValue => proxy.$disabledStartDate(startValue, queryForm.reportDateEnd)" />
                                </a-form-item>
                            </a-col>
                            <a-col v-bind="colLayout.cols">
                                <a-form-item label="申报月份(止)">
                                    <a-date-picker v-model:value="queryForm.reportDateEnd" format="YYYY-MM"
                                        value-format="YYYY-MM" placeholder="请选择申报月份(止)" picker="month"
            ...
            <!-- 详情页面弹窗 -->
            <tpm-oee-detail v-if="showDetailModal" ref="detailModal" :form-id="formId" @close="showDetailModal = false" />
            <AvicExcelImport v-if="showImportModal" :formData="excelParams" title="模板导入"
                importUrl="/mms/tpm/tpmoees/importData/v1" downloadTemplateUrl="/mms/tpm/tpmoees/downloadTemplate/v1"
...
import { listTpmOeeByPage, delTpmOee, exportExcel } from '@/api/avic/mms/tpm/TpmOeeApi'; // 引入模块API
import TpmOeeAdd from './TpmOeeAdd.vue'; // 引入添加页面组件
import TpmOeeEdit from './TpmOeeEdit.vue'; // 引入编辑页面组件
...
const excelParams = ref({ tableName: 'tpmOee' }); // 必填参数tableName全局唯一，与tableKey保持一致
...
function handleQuery() {
    if (queryForm.value.reportDateBegin != undefined) {
        queryForm.value.reportDateBegin = dayjs(queryForm.value.reportDateBegin.toString() + '-02')
    }
    if (queryForm.value.reportDateEnd != undefined) {
        queryForm.value.reportDateEnd = dayjs(queryForm.value.reportDateEnd.toString() + '-31')
    }
