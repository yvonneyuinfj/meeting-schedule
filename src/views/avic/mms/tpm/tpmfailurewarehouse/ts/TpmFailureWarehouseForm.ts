...
export function useTpmFailureWarehouseForm({ props: props, emit: emit }) {
  ...
  const rules: Record<string, Rule[]> = {
    ...
    faultDate: [
      { required: true, message: '故障发生时间不能为空', trigger: 'change' }
    ],
    planFinishDate: [
      { required: true, message: '计划完成时间不能为空', trigger: 'change' }
    ],
    failureWarehouseType: [
      { required: true, message: '故障库类型不能为空', trigger: 'change' }
    ],
    secretLevel: [
      { required: true, message: '密级不能为空', trigger: 'change' }
    ],
    faultContent: [
      { required: true, message: '故障现象及原因不能为空', trigger: 'change' }
    ],
    repairContent: [
