...
import { getWmsInvOutBill, saveWmsInvOutBill, saveFormAndStartProcess } from '@/api/avic/mms/wms/WmsInvOutBillApi'; // 引入模块API
import {
  startFlowByFormCode,
  closeFlowLoading,
  openFlowDetail,
...
export function useWmsInvOutBillForm({ props: props, emit: emit }) {
  const { proxy } = getCurrentInstance();
  const form = ref<WmsInvOutBillDto>({});
  const formRef = ref(null);
  const formCode = 'WmsInvOutBill';
      ...
      .then(async () => {
        wmsInvOutBillLEdit.value
          .validate(async validate => {
            ...
            const subInfoList = wmsInvOutBillLEdit.value.getChangedData(); // 获取子表数据
            // 处理数据
            postData.wmsInvOutBillLList = subInfoList; // 挂载子表数据
            // 发送请求
            saveWmsInvOutBill(postData)
              .then(res => {
                if (res.success) {
