<template>
  <AvicSplit horizontal>
    <AvicPane size="55">
      <div class="content-wrapper">
          ...
          <a-form v-bind="layout" ref="formRef" :model="queryForm">
            <a-row :gutter="16">
              ...
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="调入部门ID">
                  <AvicCommonSelect
                    v-model:value="queryForm.inTransferDeptId"
                    type="deptSelect"
                    placeholder="请选择调入部门ID"
                    :defaultShowValue="queryForm.inTransferDeptIdAlias"
                    @callback="
                      result => {
                        queryForm.inTransferDeptIdAlias = result.names;
                      }
                    "
                  />
                </a-form-item>
              ...
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="调入原因">
                  <a-input
                    v-model:value="queryForm.borrowReason"
                    placeholder="请输入调入原因"
                    :allow-clear="true"
                    @pressEnter="handleQuery"
                  />
              ...
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="调出部门ID">
                  <AvicCommonSelect
                    v-model:value="queryForm.outTransferDeptId"
                    type="deptSelect"
                    placeholder="请选择调出部门ID"
                    :defaultShowValue="queryForm.outTransferDeptIdAlias"
                    @callback="
                      result => {
                        queryForm.outTransferDeptIdAlias = result.names;
                      }
                    "
                  />
                </a-form-item>
        ...
        <div class="table-wrapper">
          <AvicTable
            ref="famAssetTransfer"
            table-key="famAssetTransfer"
            :columns="columns"
          ...
          <a-space>
            <a-button
              v-hasPermi="['famAssetTransfer:add']"
              title="添加"
              type="primary"
              @click="handleAdd"
            >
            ...
            <a-button
              v-hasPermi="['famAssetTransfer:edit']"
              title="编辑"
              type="primary"
              ghost
              @click="handleEdit"
            ...
            <a-button
              v-hasPermi="['famAssetTransfer:del']"
              title="删除"
              danger
            ...
            <a-button
              v-hasPermi="['famAssetTransfer:export']"
              title="导出"
              type="primary"
              ghost
              @click="handleExport">
            ...
            <template #toolBarRight>
              <a-space>
                <AvicBpmFilter
                  :allFileAuth="['famAssetTransfer:all']"
                  :myFileAuth="['famAssetTransfer:my']"
    ...
    <AvicPane>
      <!--子表组件-->
      <FamAssetTransferListManage
        key="famAssetTransferListManage"
        ref="famAssetTransferListManage"
        :mainId="mainId"
      />
    </AvicPane>
  </AvicSplit>
...
import type { FamAssetTransferDto } from '@/api/avic/mms/fam/FamAssetTransferApi'; // 引入模块DTO
import { listFamAssetTransferByPage, delFamAssetTransfer, exportExcel } from '@/api/avic/mms/fam/FamAssetTransferApi'; // 引入模块API
import FamAssetTransferAdd from './FamAssetTransferAdd.vue'; // 引入添加页面组件
import FamAssetTransferEdit from './FamAssetTransferEdit.vue'; // 引入编辑页面组件
import FamAssetTransferDetail from './FamAssetTransferDetail.vue'; // 引入详情页面组件
import FamAssetTransferListManage from '../famassettransferlist/FamAssetTransferListManage.vue'; // 引入子表页面组件
...
const columns = [
  ...
  {
    title: '调出部门ID',
    dataIndex: 'outTransferDeptIdAlias',
    ellipsis: true,
    minWidth: 120,
    resizable: true,
    align: 'left'
...
function getList() {
  ...
  selectedRows.value = [];
  loading.value = true;
  listFamAssetTransferByPage(queryParam)
    .then(response => {
