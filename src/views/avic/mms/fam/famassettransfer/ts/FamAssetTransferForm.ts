...
import { getFamAssetTransfer, saveFamAssetTransfer, saveFormAndStartProcess } from '@/api/avic/mms/fam/FamAssetTransferApi'; // 引入模块API
import {
...
export function useFamAssetTransferForm({ props: props, emit: emit }) {
  ...
  const form = ref<FamAssetTransferDto>({});
  const formRef = ref(null);
  const formCode = 'FamAssetTransfer';
  ...
  const famAssetTransferListEdit = ref();
  const layout = {
    labelCol: { flex: '140px' },
    wrapperCol: { flex: '1' }
  };
  ...
  function saveForm(params) {
    formRef.value
      .validate()
      .then(async () => {
        famAssetTransferListEdit.value
          .validate(async validate => {
            ...
            const subInfoList = famAssetTransferListEdit.value.getChangedData(); // 获取子表数据
            // 处理数据
            postData.famAssetTransferListList = subInfoList; // 挂载子表数据
            // 发送请求
            saveFamAssetTransfer(postData)
              .then(res => {
                if (res.success) {
