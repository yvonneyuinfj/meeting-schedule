  ...
  <div style="padding-bottom: 8px">
    ...
    >
      ...
      <template #bodyCell="{ column, text, record }">
          <AvicRowEdit
           v-if="['assetCode','assetOriginalValue','attachmentToolList','scrapReason','assetModel','assetSecretLevel','assetSpec','classifiedStorageMediaList','assetNetValue','handleType','factorySerialNumber','assetName','handleStatus','equipNo'].includes(
               column.dataIndex
              )"
            :record="record"
...
import { listFamHandleListByPage } from '@/api/avic/mms/fam/FamHandleListApi'; // 引入模块API
...
const columns = [
  ...
  {
    title: '涉密存储介质清单',
    dataIndex: 'classifiedStorageMediaList',
    key: 'classifiedStorageMediaList',
    ellipsis: true,
    minWidth: 120,
    resizable: true,
    align: 'left'
...
const queryForm = ref<FamHandleListDto>({});
...
function getList() {
  ...
  selectedRows.value = [];
  loading.value = true;
  queryForm.value.handleId = props.mainId ? props.mainId : '-1';
  queryParam.searchParams = queryForm.value;
  listFamHandleListByPage(queryParam)
    .then(response => {
...
function validateRecordData(records) {
  ...
  for (let index in records) {
    flag = proxy.$validateRecordData(records[index], validateRules, list.value, famHandleList);
    if (!flag) {
      break;
    }
  }
