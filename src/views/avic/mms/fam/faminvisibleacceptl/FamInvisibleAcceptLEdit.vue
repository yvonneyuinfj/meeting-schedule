<template>
  ...
  <div style="padding-bottom: 8px">
    <AvicTable ref="famInvisibleAcceptL" table-key="famInvisibleAcceptL" :height="300" :columns="columns"
      ...
      <template v-if="!props.readOnly" #toolBarLeft>
        <a-space>
          <a-space>
            <a-button v-hasPermi="['famInvisibleAcceptL:add']" title="添加" type="primary" @click="handleAdd">
              <template #icon>
                <plus-outlined />
              </template>
              添加
            </a-button>
            <a-button v-hasPermi="['famInvisibleAcceptL:del']" title="删除" danger
      ...
      <template #bodyCell="{ column, text, record }">
        <AvicRowEdit v-if="['usufruct', 'ownerShip', 'softwareVendors', 'factoryPrice', 'assetName', 'assetNo', 'entity', 'usufructNo'].includes(
          column.dataIndex
        ...
        <AvicRowEdit v-else-if="column.dataIndex === 'ynAuthorization'" :record="record" :column="column.dataIndex">
          <template #edit>
            <a-select v-model:value="record.ynAuthorization" style="width: 100%" placeholder="请选择是否授权"
              @change="(value) => changeControlValue(value, record, 'ynAuthorization')">
              <a-select-option v-for="select in ynAuthorizationList" :key="select.sysLookupTlId"
          ...
          <template #default>
            <AvicDictTag :value="record.ynAuthorizationName" :options="ynAuthorizationList" />
          </template>
        </AvicRowEdit>
        <AvicRowEdit v-else-if="column.dataIndex === 'num'" :record="record" :column="column.dataIndex">
          <template #edit>
            <a-input-number v-model:value="record.num" :min="0" :max="999999999999" :precision="0" :step="1"
              style="width: 100%" placeholder="请输入数量（默认1）">
            </a-input-number>
          </template>
        </AvicRowEdit>
        <AvicRowEdit v-else-if="column.dataIndex === 'useTime'" :record="record" :column="column.dataIndex">
          <template #edit>
            <a-input-number v-model:value="record.useTime" :min="0" :max="999999999999" :precision="0" :step="1"
              style="width: 100%" placeholder="请输入使用年限（10年）">
            </a-input-number>
          </template>
        </AvicRowEdit>
...
import type { FamInvisibleAcceptLDto } from '@/api/avic/mms/fam/FamInvisibleAcceptLApi'; // 引入模块DTO
import { listFamInvisibleAcceptLByPage } from '@/api/avic/mms/fam/FamInvisibleAcceptLApi'; // 引入模块API
...
const queryForm = ref<FamInvisibleAcceptLDto>({});
...
const famInvisibleAcceptL = ref(null);
...
function getList() {
  ...
  selectedRows.value = [];
  loading.value = true;
  queryForm.value.famInvisibleAcceptId = props.mainId ? props.mainId : '-1';
...
function validateRecordData(records) {
  ...
  for (let index in records) {
    flag = proxy.$validateRecordData(records[index], validateRules, list.value, famInvisibleAcceptL);
    if (!flag) {
      break;
    }
  }
