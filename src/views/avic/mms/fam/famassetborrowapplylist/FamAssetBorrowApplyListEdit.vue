<template>
  ...
  <div style="padding-bottom: 8px">
    <AvicTable
      ref="famAssetBorrowApplyList"
      table-key="famAssetBorrowApplyList"
      :height="300"
      :columns="columns"
      ...
      <template v-if="!props.readOnly" #toolBarLeft>
        <a-space>
          <a-space>
            ...
            <a-button
              v-hasPermi="['famAssetBorrowApplyList:del']"
              title="删除"
              danger
...
import type { FamAssetBorrowApplyListDto } from '@/api/avic/mms/fam/FamAssetBorrowApplyListApi'; // 引入模块DTO
import { listFamAssetBorrowApplyListByPage } from '@/api/avic/mms/fam/FamAssetBorrowApplyListApi'; // 引入模块API
...
const queryForm = ref<FamAssetBorrowApplyListDto>({});
...
const famAssetBorrowApplyList = ref(null);
...
function getList() {
  ...
  selectedRows.value = [];
  loading.value = true;
  queryForm.value.borrwApplyId = props.mainId ? props.mainId : '-1';
  queryParam.searchParams = queryForm.value;
  listFamAssetBorrowApplyListByPage(queryParam)
    .then(response => {
