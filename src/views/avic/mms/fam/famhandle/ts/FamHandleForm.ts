...
import { getFamHandle, saveFamHandle, saveFormAndStartProcess } from '@/api/avic/mms/fam/FamHandleApi'; // 引入模块API
import {
...
export function useFamHandleForm({ props: props, emit: emit }) {
  const { proxy } = getCurrentInstance();
  const form = ref<FamHandleDto>({});
  const formRef = ref(null);
  const formCode = 'FamHandle';
  ...
  const rules: Record<string, Rule[]> = {
    ...
    applicationDocumentNumber: [
      { required: true, message: '申请文件号不能为空', trigger: 'change' }
    ],
    approvalNumber: [
      ...
      .then(async () => {
        famHandleListEdit.value
          .validate(async validate => {
            ...
            const subInfoList = famHandleListEdit.value.getChangedData(); // 获取子表数据
            // 处理数据
            postData.famHandleListList = subInfoList; // 挂载子表数据
            // 发送请求
            saveFamHandle(postData)
              .then(res => {
                if (res.success) {
