<template>
  <AvicSplit horizontal>
        ...
        <div class="top-search-box">
          ...
          <a-form v-bind="layout" ref="formRef" :model="queryForm">
            <a-row :gutter="16">
              ...
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="处置部门ID">
                  <AvicCommonSelect
                    v-model:value="queryForm.handleDeptId"
                    type="deptSelect"
                    placeholder="请选择处置部门ID"
                    :defaultShowValue="queryForm.handleDeptIdAlias"
                    @callback="
                      result => {
                        queryForm.handleDeptIdAlias = result.names;
                      }
                    "
                  />
                </a-form-item>
              ...
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="处置部门名称">
                  <a-input
                    v-model:value="queryForm.handleDeptName"
              ...
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="资产更新替换方案">
                  <a-input
                    v-model:value="queryForm.assetAlternateSolution"
              ...
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="申请文件号">
                  <a-input
                    v-model:value="queryForm.applicationDocumentNumber"
              ...
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="批复文号">
                  <a-input
                    v-model:value="queryForm.approvalNumber"
                    placeholder="请输入批复文号"
                    :allow-clear="true"
                    @pressEnter="handleQuery"
                  />
        ...
        <div class="table-wrapper">
          ...
          <a-space>
            ...
            <template #toolBarRight>
              <a-space>
                <AvicBpmFilter
                  :allFileAuth="['famHandle:all']"
                  :myFileAuth="['famHandle:my']"
    ...
    <AvicPane>
      <!--子表组件-->
      <FamHandleListManage
        key="famHandleListManage"
        ref="famHandleListManage"
        :mainId="mainId"
      />
    </AvicPane>
  </AvicSplit>
...
import type { FamHandleDto } from '@/api/avic/mms/fam/FamHandleApi'; // 引入模块DTO
import { listFamHandleByPage, delFamHandle, exportExcel } from '@/api/avic/mms/fam/FamHandleApi'; // 引入模块API
import FamHandleAdd from './FamHandleAdd.vue'; // 引入添加页面组件
import FamHandleEdit from './FamHandleEdit.vue'; // 引入编辑页面组件
import FamHandleDetail from './FamHandleDetail.vue'; // 引入详情页面组件
import FamHandleListManage from '../famhandlelist/FamHandleListManage.vue'; // 引入子表页面组件
...
const columns = [
  ...
  {
    title: '资产更新替换方案',
    dataIndex: 'assetAlternateSolution',
    ellipsis: true,
    sorter: true,
    minWidth: 120,
    resizable: true,
  ...
  {
    title: '申请文件号',
    dataIndex: 'applicationDocumentNumber',
    ellipsis: true,
    sorter: true,
    minWidth: 120,
    resizable: true,
...
const queryForm = ref<FamHandleDto>({
  bpmState: 'all',
  bpmType: 'my'
}); // 高级查询对象
...
function getList() {
  ...
  selectedRows.value = [];
  loading.value = true;
  listFamHandleByPage(queryParam)
    .then(response => {
