    ...
    <AvicPane size="55">
      <div class="content-wrapper">
        ...
        <div class="table-wrapper">
          <AvicTable
            ref="famAssetInventoryResult"
            table-key="famAssetInventoryResult"
            :columns="columns"
          ...
          <a-space>
            <a-button
              v-hasPermi="['famAssetInventoryResult:add']"
              title="添加"
              type="primary"
              @click="handleAdd"
            >
            ...
            <a-button
              v-hasPermi="['famAssetInventoryResult:edit']"
              title="编辑"
              type="primary"
              ghost
              @click="handleEdit"
            ...
            <a-button
              v-hasPermi="['famAssetInventoryResult:del']"
              title="删除"
              danger
            ...
            <template #toolBarRight>
              <a-space>
                <AvicBpmFilter
                  :allFileAuth="['famAssetInventoryResult:all']"
                  :myFileAuth="['famAssetInventoryResult:my']"
            ...
            <template #bodyCell="{ column, text, record, index }">
              ...
              <template v-else-if="column.dataIndex === 'handlePersonIdAlias'">
                <a @click="handleFlowDetail(record)">
                  {{ record.handlePersonIdAlias }}
                </a>
              </template>
            </template>
          </AvicTable>
...
<!--    </AvicPane> -->
    <AvicPane>
      ...
      <FamAssetInventoryResultListManage
        key="famAssetInventoryResultListManage"
        ref="famAssetInventoryResultListManage"
        :mainId="mainId"
      />
    </AvicPane>
  </AvicSplit>
...
import FamAssetInventoryResultEdit from './FamAssetInventoryResultEdit.vue'; // 引入编辑页面组件
import FamAssetInventoryResultDetail from './FamAssetInventoryResultDetail.vue'; // 引入详情页面组件
import FamAssetInventoryResultListManage from '../famassetinventoryresultlist/FamAssetInventoryResultListManage.vue'; // 引入子表页面组件
...
const queryForm = ref<FamAssetInventoryResultDto>({
  bpmState: 'all',
  bpmType: 'all'
}); // 高级查询对象
...
function getList() {
  ...
  selectedRows.value = [];
  loading.value = true;
  listFamAssetInventoryResultByPage(queryParam)
    .then(response => {
...
function handleDelete(rows, ids) {
  ...
  proxy. $confirm({
    ...
    onOk: () => {
      ...
      delFamAssetInventoryResult(ids)
        .then(res => {
          if (res.success) {
