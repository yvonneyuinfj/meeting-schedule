...
import { getFamAssetInventoryResult, saveFamAssetInventoryResult, saveFormAndStartProcess } from '@/api/avic/mms/fam/FamAssetInventoryResultApi'; // 引入模块API
import {
...
export function useFamAssetInventoryResultForm({ props: props, emit: emit }) {
  const { proxy } = getCurrentInstance();
  const form = ref<FamAssetInventoryResultDto>({});
  const formRef = ref(null);
  const formCode = 'FamAssetInventoryResult';
  ...
  const famAssetInventoryResultListEdit = ref();
  const layout = {
    labelCol: { flex: '140px' },
    wrapperCol: { flex: '1' }
  };
  ...
  function saveForm(params) {
    formRef.value
      .validate()
      .then(async () => {
        famAssetInventoryResultListEdit.value
          .validate(async validate => {
            ...
            const subInfoList = famAssetInventoryResultListEdit.value.getChangedData(); // 获取子表数据
            // 处理数据
            postData.famAssetInventoryResultListList = subInfoList; // 挂载子表数据
            // 发送请求
            saveFamAssetInventoryResult(postData)
              .then(res => {
                if (res.success) {
