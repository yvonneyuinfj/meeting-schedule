<template>
  ...
  <div style="padding-bottom: 8px">
    <AvicTable ref="famInvisibleHandleL" table-key="famInvisibleHandleL" :height="300" :columns="columns"
      ...
      <template v-if="!props.readOnly" #toolBarLeft>
        <a-space>
          <a-space>
            <a-button v-hasPermi="['famInvisibleHandleL:add']" title="添加" type="primary" @click="handleAdd">
              <template #icon>
                <plus-outlined />
              </template>
              添加
...
const famInvisibleHandleL = ref(null);
...
function getList() {
  ...
  selectedRows.value = [];
  loading.value = true;
  queryForm.value.famInvisibleHandleId = props.mainId ? props.mainId : '-1';
...
function validateRecordData(records) {
  ...
  for (let index in records) {
    flag = proxy.$validateRecordData(records[index], validateRules, list.value, famInvisibleHandleL);
    if (!flag) {
      break;
    }
  }
