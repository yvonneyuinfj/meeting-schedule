<template>
  <AvicSplit horizontal>
    <AvicPane size="55">
      <div class="content-wrapper">
        <div class="top-search-box">
          ...
          <a-form v-bind="layout" ref="formRef" :model="queryForm">
            <a-row :gutter="16">
              ...
              <a-col v-bind="colLayout.cols">
                <a-form-item label="封存类型">
                  <a-input
                    v-model:value="queryForm.sealingType"
                    placeholder="请输入封存类型"
                    :allow-clear="true"
                    @pressEnter="handleQuery"
                  />
              ...
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="封存开始日期(起)">
                  <a-date-picker
                    v-model:value="queryForm.sealingStartDateBegin"
                    format="YYYY-MM-DD"
                    value-format="YYYY-MM-DD"
                    placeholder="请选择封存开始日期(起)"
                    :disabled-date="startValue => proxy.$disabledStartDate(startValue, queryForm.sealingStartDateEnd)"
                  />
                </a-form-item>
              </a-col>
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="封存开始日期(止)">
                  <a-date-picker
                    v-model:value="queryForm.sealingStartDateEnd"
                    format="YYYY-MM-DD"
                    value-format="YYYY-MM-DD"
                    placeholder="请选择封存开始日期(止)"
                    :disabled-date="endValue => proxy.$disabledEndDate(endValue, queryForm.sealingStartDateBegin)"
                  />
                </a-form-item>
              </a-col>
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="封存终止日期(起)">
                  <a-date-picker
                    v-model:value="queryForm.sealingEndDateBegin"
                    format="YYYY-MM-DD"
                    value-format="YYYY-MM-DD"
                    placeholder="请选择封存终止日期(起)"
                    :disabled-date="startValue => proxy.$disabledStartDate(startValue, queryForm.sealingEndDateEnd)"
                  />
                </a-form-item>
              </a-col>
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="封存终止日期(止)">
                  <a-date-picker
                    v-model:value="queryForm.sealingEndDateEnd"
                    format="YYYY-MM-DD"
                    value-format="YYYY-MM-DD"
                    placeholder="请选择封存终止日期(止)"
                    :disabled-date="endValue => proxy.$disabledEndDate(endValue, queryForm.sealingEndDateBegin)"
                  />
                </a-form-item>
              </a-col>
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="启封开始日期(起)">
                  <a-date-picker
                    v-model:value="queryForm.unsealStartDateBegin"
                    format="YYYY-MM-DD"
                    value-format="YYYY-MM-DD"
                    placeholder="请选择启封开始日期(起)"
                    :disabled-date="startValue => proxy.$disabledStartDate(startValue, queryForm.unsealStartDateEnd)"
                  />
                </a-form-item>
              </a-col>
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="启封开始日期(止)">
                  <a-date-picker
                    v-model:value="queryForm.unsealStartDateEnd"
                    format="YYYY-MM-DD"
                    value-format="YYYY-MM-DD"
                    placeholder="请选择启封开始日期(止)"
                    :disabled-date="endValue => proxy.$disabledEndDate(endValue, queryForm.unsealStartDateBegin)"
                  />
                </a-form-item>
              </a-col>
              <a-col v-bind="colLayout.cols" v-show="advanced">
              ...
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="封存规定执行情况">
                  <a-input
                    v-model:value="queryForm.sealingExecuteStatus"
              ...
              <a-col v-bind="colLayout.cols" v-show="advanced">
                <a-form-item label="启封执行情况">
                  <a-input
                    v-model:value="queryForm.unsealExecuteStatus"
        ...
        <div class="table-wrapper">
          <AvicTable
            ref="famSealAndUnseal"
            table-key="famSealAndUnseal"
            :columns="columns"
          ...
          <a-space>
            <a-button
              v-hasPermi="['famSealAndUnseal:add']"
              title="添加"
              type="primary"
              @click="handleAdd"
            >
            ...
            <a-button
              v-hasPermi="['famSealAndUnseal:edit']"
              title="编辑"
              type="primary"
              ghost
              @click="handleEdit"
            ...
            <a-button
              v-hasPermi="['famSealAndUnseal:del']"
              title="删除"
              danger
            ...
            <a-button
              v-hasPermi="['famSealAndUnseal:export']"
              title="导出"
              type="primary"
              ghost
              @click="handleExport">
            ...
            <template #toolBarRight>
              <a-space>
                <AvicBpmFilter
                  :allFileAuth="['famSealAndUnseal:all']"
                  :myFileAuth="['famSealAndUnseal:my']"
    ...
    <AvicPane>
      <!--子表组件-->
      <FamSealAndUnsealListManage
        key="famSealAndUnsealListManage"
        ref="famSealAndUnsealListManage"
        :mainId="mainId"
      />
    </AvicPane>
  </AvicSplit>
...
import { listFamSealAndUnsealByPage, delFamSealAndUnseal, exportExcel } from '@/api/avic/mms/fam/FamSealAndUnsealApi'; // 引入模块API
import FamSealAndUnsealAdd from './FamSealAndUnsealAdd.vue'; // 引入添加页面组件
import FamSealAndUnsealEdit from './FamSealAndUnsealEdit.vue'; // 引入编辑页面组件
import FamSealAndUnsealDetail from './FamSealAndUnsealDetail.vue'; // 引入详情页面组件
import FamSealAndUnsealListManage from '../famsealandunseallist/FamSealAndUnsealListManage.vue'; // 引入子表页面组件
...
const columns = [
  ...
  {
    title: '封存规定执行情况',
    dataIndex: 'sealingExecuteStatus',
    ellipsis: true,
    sorter: true,
    minWidth: 120,
    resizable: true,
...
const queryForm = ref<FamSealAndUnsealDto>({
  bpmState: 'all',
  bpmType: 'all'
}); // 高级查询对象
...
function getList() {
  ...
  selectedRows.value = [];
  loading.value = true;
  listFamSealAndUnsealByPage(queryParam)
    .then(response => {
