...
const queryForm = ref<FamScrapListDto>({});
...
function getList() {
  ...
  selectedRows.value = [];
  loading.value = true;
  queryForm.value.scrapId = props.mainId ? props.mainId : '-1';
  queryParam.searchParams = queryForm.value;
  listFamScrapListByPage(queryParam)
    .then(response => {
...
function validateRecordData(records) {
  ...
  for (let index in records) {
    flag = proxy.$validateRecordData(records[index], validateRules, list.value, famScrapList);
    if (!flag) {
      break;
    }
  }
