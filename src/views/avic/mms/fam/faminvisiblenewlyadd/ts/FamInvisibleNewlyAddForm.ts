...
import { getFamInvisibleNewlyAdd, saveFamInvisibleNewlyAdd, saveFormAndStartProcess } from '@/api/avic/mms/fam/FamInvisibleNewlyAddApi'; // 引入模块API
import {
  startFlowByFormCode,
  closeFlowLoading,
  openFlowDetail,
...
export function useFamInvisibleNewlyAddForm({ props: props, emit: emit }) {
  const { proxy } = getCurrentInstance();
  const form = ref<FamInvisibleNewlyAddDto>({});
  const formRef = ref(null);
  const formCode = 'FamInvisibleNewlyAdd';
  ...
  const rules: Record<string, Rule[]> = {
    ...
    invisibleAssetName: [{
      required: true, message: '无形资产名称不能为空', trigger: 'change'
    }],
    budgetNum: [{
      required: true, message: '预算号不能为空', trigger: 'change'
    }],
    budgetUnitPrice: [{
      required: true, message: '预计单价（万元）不能为空', trigger: 'change'
    }],
    managerDeptId: [{
      required: true, message: '主管部门不能为空', trigger: 'change'
    }],
    managerLeaderId: [{
      required: true, message: '主管副所长不能为空', trigger: 'change'
    }]
  };
  const layout = {
  ...
  function saveForm(params) {
    formRef.value
      ...
      .then(() => {
        ...
        // 发送请求
        saveFamInvisibleNewlyAdd(postData)
          .then(res => {
            if (res.success) {
