...
import { listFamAssetInventoryResultListByPage, delFamAssetInventoryResultList } from '@/api/avic/mms/fam/FamAssetInventoryResultListApi'; // 引入模块API
...
const columns = [
  ...
  {
    title: '盘点结果（正常/盘盈/盘亏）',
    dataIndex: 'inventoryResultsName',
    ellipsis: true,
    minWidth: 120,
    resizable: true,
    align: 'center'
...
function getList () {
  ...
  selectedRows.value = [];
  loading.value = true;
  queryParam.searchParams.inventoryResultId = props.mainId ? props.mainId : '-1';
...
function handleDelete (ids, type) {
  ...
  proxy.$confirm({
    ...
    onOk: () => {
      ...
      delFamAssetInventoryResultList(ids)
        .then(res => {
          if (res.success) {
