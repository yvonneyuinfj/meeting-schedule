...
import { getFamInvisibleHandle, saveFamInvisibleHandle, saveFormAndStartProcess } from '@/api/avic/mms/fam/FamInvisibleHandleApi'; // 引入模块API
import {
...
export function useFamInvisibleHandleForm({ props: props, emit: emit }) {
  const { proxy } = getCurrentInstance();
  const form = ref<FamInvisibleHandleDto>({});
  const formRef = ref(null);
  const formCode = 'FamInvisibleHandle';
  ...
  const famInvisibleHandleLEdit = ref();
  const layout = {
    labelCol: { flex: '140px' },
    wrapperCol: { flex: '1' }
  };
      ...
      .then(async () => {
        famInvisibleHandleLEdit.value
          .validate(async validate => {
            ...
            const subInfoList = famInvisibleHandleLEdit.value.getChangedData(); // 获取子表数据
            // 处理数据
            postData.famInvisibleHandleLList = subInfoList; // 挂载子表数据
            if (famInvisibleHandleLEdit.value.list.length === 0) {
              loading.value = false;
            ...
            // 发送请求
            saveFamInvisibleHandle(postData)
              .then(res => {
                if (res.success) {
