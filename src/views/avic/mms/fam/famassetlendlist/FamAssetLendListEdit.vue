<template>
  ...
  <div style="padding-bottom: 8px">
    <AvicTable
      ref="famAssetLendList"
      table-key="famAssetLendList"
      :height="300"
      :columns="columns"
...
const queryForm = ref<FamAssetLendListDto>({});
...
const famAssetLendList = ref(null);
...
function getList() {
  ...
  selectedRows.value = [];
  loading.value = true;
  queryForm.value.assetLendId = props.mainId ? props.mainId : '-1';
  queryParam.searchParams = queryForm.value;
  listFamAssetLendListByPage(queryParam)
    .then(response => {
...
function validateRecordData(records) {
  ...
  for (let index in records) {
    flag = proxy.$validateRecordData(records[index], validateRules, list.value, famAssetLendList);
    if (!flag) {
      break;
    }
  }
