  ...
  <div style="padding-bottom: 8px">
    ...
    >
      ...
      <template #bodyCell="{ column, text, record }">
          <AvicRowEdit
           v-if="column.dataIndex === 'importedOrNot'"
            :record="record"
          ...
          <template #edit>
            <a-radio-group
              ...
              @change="(event)=>{changeControlValue(event.target.value,record,'importedOrNot')}"
            >
              <a-radio
            ...
            <template #default>
              {{ record['importedOrNotName'] }}
            </template>
          </AvicRowEdit>
          <AvicRowEdit
            v-else-if="['note','assetOriginalValue','assetModel','assetName','equipNo','assetClass','assetNo','assetSpec'].includes(
               column.dataIndex
              )"
            :record="record"
            :column="column.dataIndex"
...
const lookupParams = [
  { fieldName: 'importedOrNot', lookUpType: 'FAM_WHETHER_OR_NOT' }
];
const validateRules = {
  secretLevel: [
    { required:true, message: '数据密级列不能为空' }
  ],
  assetClass: [
