<template>
  <AvicSplit @onresize="onResize">
    <AvicPane size="250px" collapsible="left">
      <div class="tree_content" ref="treeContentRef">
        ...
        <div class="surface-search">
          <a-input-search :allow-clear="true" placeholder="输入上级资产类别名称查询" @search="handleSearchTree"
        ...
        <div class="tree_box">
          <a-spin :spinning="loading">
            <a-tree v-if="treeData && treeData.length > 0" v-model:expanded-keys="expandedKeys"
              ...
              :show-line="true && { showLeafIcon: false }" :default-expand-all="true" :height="treeHeight"
              @expand="handleExpand" @select="handleSelect" @rightClick="handleRightClick">
              <template #title="{ dataRef }">
                <a-dropdown :trigger="['contextmenu']">
                  <div v-if="dataRef.title.indexOf(searchValue) > -1" :title="dataRef.title">
                    {{ dataRef.title.substr(0, dataRef.title.indexOf(searchValue)) }}
                    <div style="color: #f50;min-width: 150px;">{{ searchValue }}</div>
                    {{
                      dataRef.title.substr(dataRef.title.indexOf(searchValue) + searchValue.length)
                    }}
                  </div>
                  <div v-else :title="dataRef.title" style="min-width:150px">{{ dataRef.title }}</div>
                  <template #overlay>
                    <a-menu @click="({ key: menuKey }) => handleContextMenuClick(dataRef, menuKey)">
                      <a-menu-item key="1"
                        v-if="getContextMenuVisiable(dataRef, '1') && proxy.$hasPermi(['famAssetClass:add'])">
                        添加平级节点
                      </a-menu-item>
                      <a-menu-item key="2" v-if="proxy.$hasPermi(['famAssetClass:add'])">
                        添加子节点
                      </a-menu-item>
                      <a-menu-item key="3" v-if="proxy.$hasPermi(['famAssetClass:edit'])">
                        编辑节点
                      </a-menu-item>
                      <a-menu-item key="4" v-if="proxy.$hasPermi(['famAssetClass:del'])">
                        删除节点
                      </a-menu-item>
                    </a-menu>
                  </template>
    ...
    <FamAssetClassEdit v-if="showEditNodeModal" :formId="nodeId" :parentId="parentId" :parentTitle="parentTitle"
      @afterEditTreeNode="afterEditTreeNode" @close="showEditNodeModal = false" />
  </AvicSplit>
</template>
...
import {
  ...
  searchTreeList,
  delFamAssetClass
} from '@/api/avic/mms/fam/FamAssetClassApi'; // 引入模块API
import {
  addSameNode,
  addSubNode,
  getExpandedKeys,
  ...
  proxy.$confirm({
    ...
    onOk: () => {
      if (selectNodeData.id) {
        delFamAssetClass(selectNodeData.id)
          .then(res => {
            if (res.success) {
