<template>
  <AvicSplit horizontal>
    ...
    <AvicPane>
      <!--子表组件-->
      <FamAccpetListManage key="famAccpetListManage" ref="famAccpetListManage" :mainId="mainId" :mainBpm="mainbpm"/>
    </AvicPane>
  </AvicSplit>
</template>
...
const mainbpm = computed(() => {
  return selectedRows.value.length === 1 ? selectedRows.value[0].businessstate_ : ''; // 主表传入子表的id
});
