<template>
  <AvicSplit @onresize="onResize">
                ...
                <a-dropdown :trigger="['contextmenu']">
                  ...
                  <template #overlay>
                    <a-menu @click="({ key: menuKey }) => handleContextMenuClick(dataRef, menuKey)">
                      <a-menu-item key="1" v-if="getContextMenuVisiable(dataRef, '1') && proxy.$hasPermi(['famAssetClassTherr:add'])">
                        添加平级节点
                      </a-menu-item>
                      <a-menu-item key="2" v-if="proxy.$hasPermi(['famAssetClassTherr:add'])">
                        添加子节点
                      </a-menu-item>
                      <a-menu-item key="3" v-if="proxy.$hasPermi(['famAssetClassTherr:edit'])">
                        编辑节点
                      </a-menu-item>
                      <a-menu-item key="4" v-if="proxy.$hasPermi(['famAssetClassTherr:del'])">
                        删除节点
                      </a-menu-item>
                    </a-menu>
                  </template>
    ...
    <AvicPane>
        <FamAssetClassManage
          ref="rightListManageRef"
          :mainId="nodeId"
        />
    </AvicPane>
    <!-- 添加页面弹窗 -->
    <FamAssetClassTherrAdd
...
import {
  getTreeData,
  searchTreeList,
  delFamAssetClassTherr
} from '@/api/avic/mms/fam/FamAssetClassTherrApi'; // 引入模块API
import {
  addSameNode,
  addSubNode,
  getExpandedKeys,
...
import FamAssetClassTherrAdd from './FamAssetClassTherrAdd.vue'; // 引入添加页面组件
import FamAssetClassTherrEdit from './FamAssetClassTherrEdit.vue'; // 引入编辑页面组件
import FamAssetClassManage from '../famassetclass/FamAssetClassManage.vue'; // 引入子表页面组件
  ...
  proxy.$confirm({
    ...
    onOk: () => {
      if (selectNodeData.id) {
        delFamAssetClassTherr(selectNodeData.id)
          .then(res => {
            if (res.success) {
