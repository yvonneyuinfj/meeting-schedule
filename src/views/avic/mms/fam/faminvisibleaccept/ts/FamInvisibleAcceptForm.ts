...
import { getFamInvisibleAccept, saveFamInvisibleAccept, saveFormAndStartProcess } from '@/api/avic/mms/fam/FamInvisibleAcceptApi'; // 引入模块API
import {
...
export function useFamInvisibleAcceptForm({ props: props, emit: emit }) {
  const { proxy } = getCurrentInstance();
  const form = ref<FamInvisibleAcceptDto>({});
  const formRef = ref(null);
  const formCode = 'FamInvisibleAccept';
  ...
  const famInvisibleAcceptLEdit = ref();
  const layout = {
    labelCol: { flex: '140px' },
    wrapperCol: { flex: '1' }
  };
      ...
      .then(async () => {
        famInvisibleAcceptLEdit.value
          .validate(async validate => {
            ...
            const subInfoList = famInvisibleAcceptLEdit.value.getChangedData(); // 获取子表数据
            // 处理数据
            postData.famInvisibleAcceptLList = subInfoList; // 挂载子表数据
            if (famInvisibleAcceptLEdit.value.list.length === 0) {
              loading.value = false;
            ...
            // 发送请求
            saveFamInvisibleAccept(postData)
              .then(res => {
                if (res.success) {
