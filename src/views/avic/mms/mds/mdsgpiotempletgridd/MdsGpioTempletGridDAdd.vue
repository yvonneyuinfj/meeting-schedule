      ...
      <a-form ref="formRef" :model="form" :rules="rules" v-bind="layout" layout="horizontal">
        <a-row :gutter="16">
          <a-col v-bind="colLayout.cols">
            <a-form-item name="fieldName" label="字段名称" has-feedback>
              <a-input v-model:value="form.fieldName" :maxLength="512" :auto-focus="true" placeholder="请输入字段名称" />
            </a-form-item>
          </a-col>
          <a-col v-bind="colLayout.cols">
            <a-form-item name="fieldDesc" label="字段描述">
              <a-input v-model:value="form.fieldDesc" :maxLength="2048" placeholder="请输入字段描述" />
            </a-form-item>
          </a-col>
          <a-col v-bind="colLayout.cols">
            <a-form-item name="columnNo" label="列数" has-feedback>
              <a-input-number v-model:value="form.columnNo" :min="0" :max="99" :precision="0" :step="1"
                placeholder="请输入列数" />
            </a-form-item>
          </a-col>
          <a-col v-bind="colLayout.cols">
...
import { useMdsGpioTempletGridDForm, emits } from './ts/MdsGpioTempletGridDForm'; // 引入表单ts
