<template>
  <div class="content-wrapper">
    ...
    <div class="table-wrapper">
      <AvicTable ref="mdsInventoryType" table-key="mdsInventoryType" :columns="columns" :row-key="record => record.id"
        ...
        <template #toolBarLeft>
          <a-space>
            <a-button v-hasPermi="['mdsInventoryType:add']" title="添加" type="primary" @click="handleAdd">
              <template #icon>
                <plus-outlined />
              </template>
              添加
            </a-button>
            <a-button v-hasPermi="['mdsInventoryType:del']" title="删除" danger
            ...
            <a-button v-hasPermi="['mdsInventoryType:import']" title="导入" type="primary" ghost @click="handleImport">
              <template #icon>
                <import-outlined />
              </template>
              导入
            </a-button>
            <a-button v-hasPermi="['mdsInventoryType:export']" title="导出" type="primary" ghost @click="handleExport">
              <template #icon>
                <export-outlined />
              </template>
              导出
        ...
        <template #bodyCell="{ column, text, record, index }">
          ...
          <template v-else-if="column.dataIndex === 'invTypeCode'">
          ...
          <template v-else-if="column.dataIndex === 'action'">
            ...
            <a-button v-hasPermi="['mdsInventoryType:del']" type="link" class="inner-btn"
    ...
    <!-- 添加页面弹窗 -->
    <mds-inventory-type-add v-if="showAddModal" ref="addModal" @reloadData="getList" @close="showAddModal = false" />
    <!-- 编辑页面弹窗 -->
    <mds-inventory-type-edit v-if="showEditModal" ref="editModal" :form-id="formId" @reloadData="getList"
      @close="showEditModal = false" />
    <!-- 详情页面弹窗 -->
    <mds-inventory-type-detail v-if="showDetailModal" ref="detailModal" :form-id="formId"
      @close="showDetailModal = false" />
    <AvicExcelImport v-if="showImportModal" :formData="excelParams" title="单表模板导入"
      importUrl="/mms/tpm/mdsinventorytypes/importData/v1"
      downloadTemplateUrl="/mms/tpm/mdsinventorytypes/downloadTemplate/v1" @reloadData="getList"
      @close="showImportModal = false" />
  </div>
</template>
<script lang="ts" setup>
import type { MdsInventoryTypeDto } from '@/api/avic/mms/mds/MdsInventoryTypeApi'; // 引入模块DTO
import { listMdsInventoryTypeByPage, delMdsInventoryType, exportExcel } from '@/api/avic/mms/mds/MdsInventoryTypeApi'; // 引入模块API
import MdsInventoryTypeAdd from './MdsInventoryTypeAdd.vue'; // 引入添加页面组件
import MdsInventoryTypeEdit from './MdsInventoryTypeEdit.vue'; // 引入编辑页面组件
import MdsInventoryTypeDetail from './MdsInventoryTypeDetail.vue'; // 引入详情页面组件
...
const excelParams = ref({ tableName: 'mdsInventoryType' }); // 导入Excel数据过滤参数
