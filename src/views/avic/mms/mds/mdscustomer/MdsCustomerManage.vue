  ...
  <AvicSplit horizontal>
    <AvicPane size="55">
      ...
      <AvicExcelImport
        ...
        title="模板导入"
        importUrl="/mms/mds/mdscustomers/importData/v1"
        downloadTemplateUrl="/mms/mds/mdscustomers/downloadTemplate/v1"
        @reloadData="getList"
        @close="showImportModal = false"
      />
    </AvicPane>
    <AvicPane>
      <mds-customer-tab-manage key="mdsCustomerTabManage" ref="mdsCustomerTabManage" :mainId="mainId"/>
    </AvicPane>
  </AvicSplit>
</template>
...
import { listMdsCustomerByPage, delMdsCustomer, exportExcel } from '@/api/avic/mms/mds/MdsCustomerApi'; // 引入模块API
import MdsCustomerAdd from './MdsCustomerAdd.vue'; // 引入添加页面组件
import MdsCustomerEdit from './MdsCustomerEdit.vue'; // 引入编辑页面组件
import MdsCustomerDetail from './MdsCustomerDetail.vue'; // 引入详情页面组件
import MdsCustomerTabManage from './MdsCustomerTabManage.vue'; // 引入子表页面组件
...
const excelParams = ref({ tableName: 'mdsCustomer' }); // 必填参数tableName全局唯一，与tableKey保持一致
