<template>
  <AvicModal :visible="true" title="添加" width="960px" height="520px" :centered="true" @cancel="closeModal">
        ...
        <a-row :gutter="16">
          <a-col v-bind="colLayout.cols" hidden>
            <a-form-item name="parentMdsInventoryId" label="上级库房ID">
              <a-input v-model:value="form.parentMdsInventoryId">
              </a-input>
            </a-form-item>
          </a-col>
          <a-col v-bind="colLayout.cols">
            <a-form-item name="parentMdsInventoryIdAlias" label="上级库房">
              <a-input v-model:value="form.parentMdsInventoryIdAlias" placeholder="选择上级库房" @click="inventoryClick">
                <template #suffix>
                  <a-tooltip title="库房">
          ...
          <a-col v-bind="colLayout.cols" hidden>
            <a-form-item name="mdsProjectId" label="项目ID">
              <a-input v-model:value="form.mdsProjectId">
              </a-input>
            </a-form-item>
          </a-col>
          <a-col v-bind="colLayout.cols">
            <a-form-item name="mdsProjectIdAlias" label="项目">
              <a-input v-model:value="form.mdsProjectIdAlias" placeholder="选择项目" @click="projectClick">
                <template #suffix>
                  <a-tooltip title="项目">
                    <ApartmentOutlined style="color: rgba(0, 0, 0, 0.45)" />
                  </a-tooltip>
    ...
    <MdsProjectSelect v-if="mdsProjectModal" ref="mdsProjectSelect" :visible="mdsProjectModal" @closeCancel="closeProject"
...
const mdsProjectModal = ref<boolean>(false);// 是否展示项目弹窗
const mdsProjectSelect = ref(null);
...
/** 项目弹窗 */
const projectClick = () => {
  mdsProjectModal.value = true;
};
/** 关闭项目弹窗 */
const closeProject = () => {
  mdsProjectModal.value = false;
};
...
const getProjectId = (v) => {
  form.value.mdsProjectId = v.id;
  form.value.mdsProjectIdAlias = v.projectName;
  mdsInventoryModal.value = false;
};
</script>
