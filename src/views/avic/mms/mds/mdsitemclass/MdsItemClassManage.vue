<template>
  <AvicSplit @onresize="onResize">
                ...
                <a-dropdown :trigger="['contextmenu']">
                  ...
                  <template #overlay>
                    <a-menu @click="({ key: menuKey }) => handleContextMenuClick(dataRef, menuKey)">
                      <a-menu-item key="1" v-if="getContextMenuVisiable(dataRef, '1') && proxy.$hasPermi(['mdsItemClass:add'])">
                        添加平级节点
                      </a-menu-item>
                      <a-menu-item key="2" v-if="proxy.$hasPermi(['mdsItemClass:add'])">
                        添加子节点
                      </a-menu-item>
                      <a-menu-item key="3" v-if="proxy.$hasPermi(['mdsItemClass:edit'])">
                        编辑节点
                      </a-menu-item>
                      <a-menu-item key="4" v-if="proxy.$hasPermi(['mdsItemClass:del'])">
                        删除节点
                      </a-menu-item>
                    </a-menu>
                  </template>
    ...
    <AvicPane>
      <MdsItemClassRightManage
        ref="mdsItemClassRightManageRef"
        :formId="nodeId"
...
import {
  ...
  searchTreeList,
  delMdsItemClass
} from '@/api/avic/mms/mds/MdsItemClassApi'; // 引入模块API
import {
  addSameNode,
  addSubNode,
  getExpandedKeys,
...
import MdsItemClassRightManage from './MdsItemClassRightManage.vue'; // 引入详细页面组件
...
const mdsItemClassRightManage = ref(null);
onMounted(() => {
  // 加载树数据
  getTreeList();
  // 重置树高度
...
function selectNode(keys: string[], { node }, type) {
  ...
  if (!type) {
    ...
    if (parentId.value == defaultRootParentId.value) {
      ...
      parentTitle.value = node.title;
      detailParentTitle.value = node.title;
    }
  } else {
...
function afterEditTreeNode(data) {
  if (currentSelectedNode.value.dataRef.parentId != data.parentId) {
    ...
    // 节点编辑成功后重新加载右侧详细区数据
    mdsItemClassRightManage.value.getList();
  }
}
/** 树节点删除方法 */
  ...
  proxy.$confirm({
    ...
    onOk: () => {
      if (selectNodeData.id) {
        delMdsItemClass(selectNodeData.id)
          .then(res => {
            if (res.success) {
