<template>
  ...
  <div style="padding-bottom: 8px">
    <AvicTable
      ref="mdsSubstitutionL"
      table-key="mdsSubstitutionL"
      :height="300"
      :columns="columns"
    ...
    >
      <template v-if="!props.readOnly" #toolBarLeft>
        <a-space>
          <a-space>
            <a-button
              v-hasPermi="['mdsSubstitutionL:add']"
              title="添加"
              type="primary"
              @click="handleAdd"
            >
            ...
            <a-button
              v-hasPermi="['mdsSubstitutionL:del']"
              title="删除"
              danger
      ...
      <template #bodyCell="{ column, text, record }">
          <AvicRowEdit
           v-if="['proItemId','proItemCode','proItemName'].includes(
               column.dataIndex
              )"
            :record="record"
...
import { MdsItemDto } from '@/api/avic/mms/mds/MdsItemApi';
import type { MdsSubstitutionLDto } from '@/api/avic/mms/mds/MdsSubstitutionLApi'; // 引入模块DTO
import { listMdsSubstitutionLByPage } from '@/api/avic/mms/mds/MdsSubstitutionLApi'; // 引入模块API
...
const queryForm = ref<MdsSubstitutionLDto>({});
...
const mdsSubstitutionL = ref(null);
...
function getList() {
  ...
  selectedRows.value = [];
  loading.value = true;
  queryForm.value.mdsSubstitutionId = props.mainId ? props.mainId : '-1';
  queryParam.searchParams = queryForm.value;
  listMdsSubstitutionLByPage(queryParam)
    .then(response => {
...
function validateRecordData(records) {
  ...
  for (let index in records) {
    flag = proxy.$validateRecordData(records[index], validateRules, list.value, mdsSubstitutionL);
    if (!flag) {
      break;
    }
  }
