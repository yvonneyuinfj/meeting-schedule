<template>
  <div class="content-wrapper">
    ...
    <div class="table-wrapper">
      <AvicTable ref="mdsProject" table-key="mdsProject" :columns="columns" :row-key="record => record.id"
        ...
        <template #toolBarLeft>
          <a-space>
            <a-button v-hasPermi="['mdsProject:add']" title="添加" type="primary" @click="handleAdd">
              <template #icon>
                <plus-outlined />
              </template>
              添加
            </a-button>
            <a-button v-hasPermi="['mdsProject:del']" title="删除" danger
            ...
            <a-button v-hasPermi="['mdsProject:import']" title="导入" type="primary" ghost @click="handleImport">
              <template #icon>
                <import-outlined />
              </template>
              导入
            </a-button>
            <a-button v-hasPermi="['mdsProject:export']" title="导出" type="primary" ghost @click="handleExport">
              <template #icon>
                <export-outlined />
              </template>
              导出
        ...
        <template #bodyCell="{ column, text, record, index }">
          ...
          <template v-else-if="column.dataIndex === 'action'">
            ...
            <a-button v-hasPermi="['mdsProject:del']" type="link" class="inner-btn"
    ...
    <!-- 添加页面弹窗 -->
    <mds-project-add v-if="showAddModal" ref="addModal" @reloadData="getList" @close="showAddModal = false" />
    <!-- 编辑页面弹窗 -->
    <mds-project-edit v-if="showEditModal" ref="editModal" :form-id="formId" @reloadData="getList"
      @close="showEditModal = false" />
    <!-- 详情页面弹窗 -->
    <mds-project-detail v-if="showDetailModal" ref="detailModal" :form-id="formId" @close="showDetailModal = false" />
    <AvicExcelImport v-if="showImportModal" :formData="excelParams" title="单表模板导入"
      importUrl="/mms/mds/mdsprojects/importData/v1" downloadTemplateUrl="/mms/mds/mdsprojects/downloadTemplate/v1"
      @reloadData="getList" @close="showImportModal = false" />
  </div>
</template>
<script lang="ts" setup>
import type { MdsProjectDto } from '@/api/avic/mms/mds/MdsProjectApi'; // 引入模块DTO
import { listMdsProjectByPage, delMdsProject, exportExcel } from '@/api/avic/mms/mds/MdsProjectApi'; // 引入模块API
import MdsProjectAdd from './MdsProjectAdd.vue'; // 引入添加页面组件
import MdsProjectEdit from './MdsProjectEdit.vue'; // 引入编辑页面组件
import MdsProjectDetail from './MdsProjectDetail.vue'; // 引入详情页面组件
...
const excelParams = ref({ tableName: 'mdsProject' }); // 导入Excel数据过滤参数
