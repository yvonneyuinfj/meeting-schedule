    ...
    <div class="table-wrapper">
      ...
      >
        <template v-slot:toolBarLeft>
          <a-space>
            <a-button type="primary" @click="batchHandleTodo()" style="margin: 0">
              <edit-outlined />
              批量办理
            </a-button>
          </a-space>
...
import { batchHandle, getBatchHandleInfo, listBpmTaskListByPage } from '@/api/avic/bpm/BpmTaskApi';
import type { TodoTaskQueryForm, treeParams, batchHandleParams } from '../types';
...
const batchHandleParam = ref<batchHandleParams>({
  // 批量办理请求接口
  entryIds: '',
  executionIds: '',
  taskIds: ''
...
/** 高级查询 重置按钮操作 */
function resetQuery() {
  queryForm.value = { taskType: '0', taskFinished: '0' } as TodoTaskQueryForm;
  handleQuery();
}
/** 高级查询 展开/收起 */
 ...
 * 批量办理
 */
function batchHandleTodo() {
  if (selectedRowKeys.value.length <= 0) {
    proxy.$message.warning('请选择要办理的记录！');
    return;
  }
  selectedRowData.value.forEach(item => {
    if (batchHandleParam.value.taskIds === '') {
      batchHandleParam.value = {
        taskIds: item.dbid,
        entryIds: item.processInstance,
        executionIds: item.executionId
      } as batchHandleParams;
    } else {
      batchHandleParam.value = {
        taskIds: batchHandleParam.value.taskIds + ',' + item.dbid,
        entryIds: batchHandleParam.value.entryIds + ',' + item.processInstance,
        executionIds: batchHandleParam.value.executionIds + ',' + item.executionId
      } as batchHandleParams;
    }
  });
  proxy.$Modal.confirm({
    title: '确定批量操作吗？',
    content: '提示：只有符合批量办理前提条件的才会被办理成功',
    okText: '确定',
    cancelText: '取消',
    onOk: () => {
      batchHandle(batchHandleParam.value)
        .then((response: any) => {
          if (response.success) {
            const [total, success, error] = response.data.result;
            if (Number(total) <= Number(success) + Number(error)) {
              proxy.$message.success(
                '操作完成！共提交了' + total + '条，成功' + success + '条，失败' + error + '条'
              );
            } else {
 ...
 * 批量办理后，请求执行结果，并刷新页面
 */
function getBatchHandlesInfo(initMessage) {
  getBatchHandleInfo()
    .then((response: any) => {
      if (response.success) {
        const [total, success, error] = response.data.result;
        if (Number(total) <= Number(success) + Number(error)) {
          ...
          if (!initMessage) {
            proxy.$message.success('正在进行批量操作...');
          }
          setTimeout(function () {
            //更新进度
            getBatchHandlesInfo(true);
          }, 1000);
        }
      }
    })
