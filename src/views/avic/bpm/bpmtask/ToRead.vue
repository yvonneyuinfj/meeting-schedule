    ...
    <div class="table-wrapper">
      ...
      >
        <template v-slot:toolBarLeft>
          <a-space>
            <a-button type="primary" @click="batchHandleToRead()" style="margin: 0">
              <edit-outlined />
              批量阅文
            </a-button>
          </a-space>
...
import { batchRead, getBatchHandleInfo, listBpmTaskListByPage } from '@/api/avic/bpm/BpmTaskApi';
...
/** 高级查询 重置按钮操作 */
function resetQuery() {
  queryForm.value = { taskType: '1', taskFinished: '0' } as TodoTaskQueryForm;
  handleQuery();
}
/** 高级查询 展开/收起 */
...
//批量已阅
function batchHandleToRead() {
  if (selectedRowKeys.value.length <= 0) {
    proxy.$message.warning('请选择要办理的记录！');
    return;
  }
  let batchHandleParam = {
    entryIds: '',
    executionIds: '',
    taskIds: ''
  };
  selectedRowData.value.forEach(item => {
    if (batchHandleParam.taskIds === '') {
      batchHandleParam = {
        taskIds: item.dbid,
      ...
      batchHandleParam = {
        taskIds: batchHandleParam.taskIds + ',' + item.dbid,
        entryIds: batchHandleParam.entryIds + ',' + item.processInstance,
        executionIds: batchHandleParam.executionIds + ',' + item.executionId
      };
    }
  });
  proxy.$Modal.confirm({
    ...
    cancelText: '取消',
    onOk: () => {
      batchRead(batchHandleParam)
        .then((response: any) => {
          if (response.success) {
            getBatchHandleInfos(false);
          }
        })
        .catch(() => {});
    }
...
function getBatchHandleInfos(initMessage) {
  getBatchHandleInfo()
    .then((response: any) => {
      if (response.success) {
        ...
        if (Number(total) <= Number(success) + Number(error)) {
          ...
          if (!initMessage) {
            proxy.$message.success('正在进行批量操作...', 1.1);
          }
          setTimeout(function () {
            //更新进度
            getBatchHandleInfos(true);
          }, 1000);
        }
      }
    })
