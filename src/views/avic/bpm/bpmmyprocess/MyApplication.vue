      ...
      >
        <template #bodyCell="{ column, text, record, index }">
          ...
          <template v-else-if="column.key === 'taskTitle'">
            <a href="javascript:;" @click.stop="handleDetail(record.dbId)">
...
let advanced = ref(false), // 高级搜索 展开/关闭
  ...
  businessStateList = ref([]); //高级查询流程状态
...
function getLookupList() {
  proxy.$getLookupByType(lookupParams, result => {
    businessStateList.value = result.businessState.filter(item => {
      return item.lookupCode != 'start';
    });
  });
}
/** 高级查询 查询按钮操作 */
...
/** 高级查询 重置按钮操作 */
function resetQuery() {
  queryForm.value = { type: '1' } as MyDraftQueryForm;
  handleQuery();
}
/** 高级查询 展开/收起 */
